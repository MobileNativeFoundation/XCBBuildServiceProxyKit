...
class MessagePackEncoderTests: XCTestCase {
    ...
    func test_encode_nonDictionaryKeyedEncodable() throws {
        XCTAssertNoDifference(
            [UInt8](try encoder.encode(
    ...
    func test_encode_Enums() throws {
        XCTAssertNoDifference(
            [UInt8](try encoder.encode(
                MessageEnum(wrappedValue: BuildOperationEnded.Status.cancelled)
            )),
            // Int(1)
        ...
        XCTAssertNoDifference(
            [UInt8](try encoder.encode(
                BuildOperationEnded(id: 42, status: .cancelled)
            )),
            // [Int(42), Int(1)]
            [0x92,  0xD3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2A,  0xD3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01]
        )
    }
}
